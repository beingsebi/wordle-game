#include <bits/stdc++.h>
using namespace std;
inline vector<pair<string, string>> get_info()
{
    ifstream f("pipe.txt");
...
inline vector<string> get_all_words()
{
...
inline bool check_in(const char &ch, const string &s)
{
    for (const char &i : s)
        if (i == ch)
            return true;
...
inline bool okk(string sx1, pair<string, string> sx2)
{
...
inline bool okay(const string &s, const vector<pair<string, string>> &v)
{
...
inline vector<string> refine_list(const vector<string> &allw)
{
    vector<string> rez;
...
inline string encode(string st, string ans)
{
    string afis = "00000";
    for (const int i : {0, 1, 2, 3, 4})
...
inline double entropy(string st, vector<string> pos)
{
    map<string, int> ma;
    string axss;
